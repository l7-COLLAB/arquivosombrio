...
async function openLiteInV2Editor(item){
 var p=item.payload||{},category=item.category,safe={},k;
 if(category==="novels"||category==="capitulos")return "redirect";
 for(k in p)if(Object.prototype.hasOwnProperty.call(p,k)&&k!=="_source"&&k!=="status_publicacao")safe[k]=p[k];
 safe.status_publicacao="rascunho";
 if(category==="lendas"||category==="creepypastas"){if(!Array.isArray(safe.conteudo_blocos)&&safe.conteudo)safe.conteudo_blocos=String(safe.conteudo).split(/\n\s*\n/).map(function(t){return{tipo:"paragrafo",texto:t.trim()}}).filter(function(t){return t.texto});if(!Array.isArray(safe.fontes)&&typeof safe.fontes==="string")safe.fontes=safe.fontes.split(/\n/).map(function(t){return t.trim()}).filter(Boolean).map(function(t){var a=t.split("|");return{titulo:a[0].trim(),url:(a[1]||"").trim()}});safe._liteImport=String(item.id);if(typeof window.arquivoAbrirEditorLiterario!=="function")throw new Error("O editor literário do V2 não carregou.");await window.arquivoAbrirEditorLiterario(category,safe);return "opened";}
 var map={
  dossies:[["titulo","admin-title"],["categoria","admin-category"],["local","admin-location"],["ano","admin-year"],["status","admin-status"],["imagem","admin-image"],["resumo","admin-summary"],["historia","admin-history"],["cronologia","admin-chronology"],["evidencias","admin-evidence"],["teorias","admin-theories"],["situacao_oficial","admin-official-status"],["fontes","admin-sources"]],
  garimpo:[["titulo","daily-title"],["categoria","daily-category"],["local","daily-location"],["data_caso","daily-date"],["status_caso","daily-case-status"],["imagem_capa","daily-cover"],["resumo","daily-summary"],["conteudo","daily-content"],["cronologia","daily-chronology"],["evidencias","daily-evidence"],["hipoteses","daily-theories"],["situacao_oficial","daily-official-status"],["fontes","daily-sources-text"]],
  pericia:[["titulo","admin-forensic-title"],["categoria","admin-forensic-category"],["resumo","admin-forensic-summary"],["imagem","admin-image"],["legenda_imagem","admin-forensic-image-caption"],["fonte_imagem","admin-forensic-image-source"],["introducao","admin-forensic-introduction"],["como_funciona","admin-forensic-operation"],["historia_tecnica","admin-forensic-history"],["aplicacao_casos_reais","admin-forensic-real-cases"],["limitacoes_controversias","admin-forensic-limitations"],["curiosidades","admin-forensic-curiosities"],["fontes","admin-forensic-sources"],["casos_relacionados","admin-forensic-related-cases"]]
 };
 if(category==="dossies"||category==="pericia"){if(typeof window.abrirFormularioAdmin!=="function")throw new Error("O editor de conteúdo do V2 não carregou.");window.abrirFormularioAdmin(category==="dossies"?"caso":"pericia");}
 else if(category==="garimpo"){if(typeof window.abrirFormularioCasoDiario!=="function")throw new Error("O editor de Garimpo do V2 não carregou.");window.abrirFormularioCasoDiario();}
 else throw new Error("Não existe formulário V2 para esta categoria.");
 window.setTimeout(function(){var fields=map[category]||[];for(var i=0;i<fields.length;i++){var input=document.getElementById(fields[i][1]),key=fields[i][0],value=p[key];if(!input||value==null)continue;if(key==="fontes"&&Array.isArray(value))input.value=value.map(function(x){return typeof x==="string"?x:[x.titulo||x.nome||"Fonte",x.url||x.link||""].filter(Boolean).join(" | ")}).join("\n");else if(Array.isArray(value))input.value=value.map(function(x){return typeof x==="string"?x:JSON.stringify(x,null,2)}).join("\n\n");else input.value=liteString(value);input.dispatchEvent(new Event("input",{bubbles:true}));input.dispatchEvent(new Event("change",{bubbles:true}));}var stateId=category==="dossies"?"admin-publicacao":category==="garimpo"?"daily-publication-status":"admin-forensic-publication",state=document.getElementById(stateId);if(state){state.value="rascunho";state.dispatchEvent(new Event("change",{bubbles:true}));}},0);
 return "opened";
}
async function liteDrafts(p){
 ...
 p.querySelectorAll("[data-lite-import]").forEach(function(b){b.onclick=async function(){var item=rows.find(function(y){return String(y.id)===b.dataset.liteImport;});if(!item)return;var wasImported=!!imported["lite:"+String(item.id)];if(!wasImported&&!confirm("Preparar este rascunho no V2? O original Lite será mantido e nada será publicado ou agendado."))return;b.disabled=true;try{if(!wasImported){await importLiteToV2(item);imported["lite:"+String(item.id)]=true;}b.textContent="Abrir no V2";var destination=await openLiteInV2Editor(item);if(destination!=="redirect")alert("O formulário correspondente do V2 foi aberto como rascunho. Confira os campos. Só a ação explícita de salvar/publicar/agendar no V2 grava o conteúdo editorial; o original Lite foi preservado.");}catch(e){alert(e.message||"Não foi possível abrir o rascunho no V2.");}finally{b.disabled=false;}};});
