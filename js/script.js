...
function abrirFormularioCasoDiario(dados = null) {
    ...
    const statusPublicacaoEl = document.getElementById("daily-publication-status");
    const agendamentoWrap = document.getElementById("daily-schedule-wrap");
    const agendamentoInput = document.getElementById("daily-scheduled-for");
    const sincronizarAgendamento = () => {
        const ativo = statusPublicacaoEl?.value === "agendado";
        if (agendamentoWrap) agendamentoWrap.hidden = !ativo;
        if (agendamentoInput) agendamentoInput.required = ativo;
    };
    statusPublicacaoEl?.addEventListener("change", sincronizarAgendamento);
...
async function carregarStoriesGarimpo() {
    ...
    if (!host) return;
    const barra = host.closest("[data-garimpo-stories-bar]");
    try {
        const cliente = await obterClienteSupabase();
        const agora = new Date().toISOString();
        ...
        if (!garimpoStoriesAtivos.length) {
            host.hidden = true;
