    ...
    itens.forEach(item => {
        ...
        if (typeof item === "string") {
            ...
            if (descricao) {
                ...
                if (!periodo) {
                    paragrafo.style.gridColumn =
                        "1 / -1";
                }

                linha.appendChild(paragrafo);
            }
...
function configurarSumarioDossie() {
    ...
    /*
     * Inclui no sumário os subtítulos e as seções especiais criadas pelo
     * editor narrativo, preservando a mesma ordem em que aparecem no dossiê.
     * IDs ausentes são criados aqui para que os links funcionem tanto no modo
     * normal quanto no modo livro.
     */
    const seletoresBlocosSumario = [
        ".case-content-subtitle",
        ".case-content-timeline",
        ".case-content-evidence",
        ".case-content-hypotheses",
    ...
    document
        .querySelectorAll(
            `#caso-historia :is(${seletoresBlocosSumario})`
        )
        .forEach((bloco, indice) => {

            if (!bloco.id) {
                bloco.id =
                    `secao-dossie-${indice + 1}`;
            }
