...
Deno.serve(async(req)=>{
 ...
 const client=async(path:string,method="GET",payload?:unknown,prefer?:string)=>{const h:Record<string,string>={apikey:key,Authorization:"Bearer "+key,"Content-Type":"application/json"};if(prefer)h.Prefer=prefer;const r=await fetch(url+"/rest/v1/"+path,{method,headers:h,body:payload===undefined?undefined:JSON.stringify(payload)});if(!r.ok)throw new Error("Database "+r.status);const responseText=await r.text();return responseText?JSON.parse(responseText):[]};
 try{
